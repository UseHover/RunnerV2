...
private data class RawStepsModel(var value: String, var description: String, var is_param: Boolean)

data class StreamlinedSteps(val fullUSSDCodeStep: String,
		...
		fun get(_rootCode: String, jsonArray: JSONArray): StreamlinedSteps {
			...
			for (rawStep in rawStepsModel) {
				...
				if (BuildConfig.FLAVOR.contains("pro")) {
					if (rawStep.is_param || rawStep.value.equals("pin")) {
