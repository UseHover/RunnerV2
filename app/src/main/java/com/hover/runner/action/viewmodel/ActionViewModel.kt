...
class ActionViewModel(private val useCase: ActionUseCase, simUseCase: SimUseCase) : AbstractFilterActionViewModel(simUseCase) {
    ...
    val networksOutsidePresentSimCountryNamesLiveData : MutableLiveData<List<String>> = MutableLiveData()

    val actionsWithUCV_LiveData: MutableLiveData<List<Action>> = MutableLiveData()  //UCV means uncompleted variables
    val actionsWithCompletedVariables: MutableLiveData<List<Action>> = MutableLiveData()

    //MediatorLiveData
    val allActions_toFind_BadActions_MediatorLiveData : MediatorLiveData<List<Action>> = MediatorLiveData()
    val badActions_toFind_GoodActions_MediatorLiveData : MediatorLiveData<List<Action>> = MediatorLiveData()
    val filterParam_toFind_FilteredActions_MediatorLiveData : MediatorLiveData<ActionFilterParam> = MediatorLiveData()

    val allNetworks_toLoad_countryCodes_MediatorLiveData : MediatorLiveData<List<String>> = MediatorLiveData()
    val countryCodes_toFind_ItsNetworks_MediatorLiveData : MediatorLiveData<List<SimInfo>> = MediatorLiveData()
    val networksWithinCountry_toFind_networksOutsideCountry_MediatorLiveData : MediatorLiveData<List<String>> = MediatorLiveData()
    ...
    private fun setupMediators() {
        allActions_toFind_BadActions_MediatorLiveData.addSource(actions, this::updateActionsWithUCVList)
        badActions_toFind_GoodActions_MediatorLiveData.addSource(actionsWithUCV_LiveData, this::updateRunnableActions)
        filterParam_toFind_FilteredActions_MediatorLiveData.addSource(actionFilterParamMutableLiveData, this::runFilter)

        allNetworks_toLoad_countryCodes_MediatorLiveData.addSource(allNetworksLiveData, this::loadSimCountries)
        countryCodes_toFind_ItsNetworks_MediatorLiveData.addSource(presentSimCountryCodes_MutableLiveData,this::loadNetworksInCountry)
        networksWithinCountry_toFind_networksOutsideCountry_MediatorLiveData.addSource(networksInPresentSimCountryNamesLiveData, this::loadNetworksOutsideCountry)
    }
    ...
    private fun loadNetworksInCountry(countryCodes: List<String>) {
        viewModelScope.launch(Dispatchers.IO) {networksInPresentSimCountryNamesLiveData.postValue(useCase.getNetworkNames(countryCodes))  }
    }
    ...
    private fun runFilter(actionFilterParam: ActionFilterParam) {
        if(!actionFilterParam.isDefault()) {
            loadingStatusLiveData.postValue(false)
            val deferredActions = viewModelScope.async(Dispatchers.IO) {return@async useCase.filter(actionFilterParam) }
