...
class UnCompletedVariablesFragment : Fragment(), ActionVariableEditListener {
    ...
    private fun setDetailsText(currentAction: Action, listSize: Int) {
        ...
        descContentText.text = String.format(Locale.getDefault(), "Action %d of %s", listSize, initialUncompletedSize)
        UIHelper.underlineText(nextSaveText, if (listSize == 1) getString(R.string.save_text) else getString(R.string.save_continue))
    }
    ...
    private fun setVariableListAdapter(action: Action) {
        try {
            ...
            UIHelper.flashMessage(requireContext(), requireActivity().currentFocus, resources.getString(R.string.bad_steps_config))
        }
    }
