...
class TransactionsFilterFragment : BaseFragment() {
    ...
    private fun setupCheckboxSelections() {
        successCheckBox.setOnCheckedChangeListener { _, checked: Boolean -> transactionViewModel.filter_IncludeSucceededTransactions(checked) }
        pendingCheckBox.setOnCheckedChangeListener{_, checked-> transactionViewModel.filter_IncludePendingTransactions(checked) }
        failureCheckBox.setOnCheckedChangeListener{_, checked-> transactionViewModel.filter_IncludeFailedTransactions(checked) }
    }
