...
import com.hover.runner.transaction.repo.TransactionRepoInterface
...
class TransactionUseCaseImpl(private val transactionRepoInterface: TransactionRepoInterface) :
	...
	override fun getAllTransactionsLiveData(): LiveData<List<RunnerTransaction>> {
		return transactionRepoInterface.getAllTransactionsLiveData()
	}
	...
	override suspend fun getDeviceInfo(runnerTransaction: RunnerTransaction): List<TransactionDetailsInfo> {
		...
		with(runnerTransaction) {
			val detailsList = mutableListOf<TransactionDetailsInfo>()
			detailsList.add(TransactionDetailsInfo("Testing mode",
			...
			detailsList.add(TransactionDetailsInfo("App ver. code",
			                                       BuildConfig.VERSION_CODE.toString(),
			                                       false))
			detailsList.add(TransactionDetailsInfo("App ver. name",
	...
	override suspend fun getDebugInfo(runnerTransaction: RunnerTransaction): List<TransactionDetailsInfo> {
		with(runnerTransaction) {
			...
			val detailsList = mutableListOf<TransactionDetailsInfo>()
			detailsList.add(TransactionDetailsInfo("Input extras",
			                                       hoverTransaction.input_extras.toString(),
			                                       false))
			detailsList.add(TransactionDetailsInfo("Matched parsers",
			                                       runnerTransaction.matched_parsers ?: "",
			                                       true))
			detailsList.add(TransactionDetailsInfo("Parsed variables",
	...
	private fun getMessages(rootCodeString: String,
	                        smsMessages: List<MessageLog>,
	                        transaction: Transaction): Array<Array<String?>?> {
	...
	private fun getMessagesModel(result: Array<Array<String?>?>): List<TransactionDetailsMessages> {
		...
		try {
			for (i in 0 until largestSize) {
				messagesModels.add(TransactionDetailsMessages(if (enteredValues[i] != null) enteredValues[i] else "",
			...
			} catch (ex: Exception) { //USE THIS FOR NO-SIM MESSAGE MODE;
